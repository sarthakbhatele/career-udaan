...
export const generateSingleIndustryInsight = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const timeSinceLastAttempt = insight.lastAttemptAt
      ? Date.now() - new Date(insight.lastAttemptAt).getTime()
      : Infinity;
    ...
    await step.run("Trigger context seeding", async () => {
      await inngest.send({
        name: "industry/insight.completed",
        data: { industry },
      });
    });

    //     // NEW: Auto-seed context chunk immediately after completion
    //     await step.run("Seed context chunk", async () => {
    ...
    //       // Store without embedding for now (Phase 5.2 adds embeddings)
